...
int ramfs_write(const char* path, const char* data, uint64_t len){
    uint64_t actual_len = len;
    if (actual_len == 0 && data) actual_len = (uint64_t)strlenz(data);
    ...
    if(!h) return -3;
    if(actual_len){ move_write(h, data, actual_len); }
    c->data = h; c->size = actual_len;
    return 0;
}

int ramfs_read(const char* path, char* out, uint64_t max, uint64_t* outLen){
    ramfs_node_t* n = ramfs_find(path);
    if(!n || n->isDir) return -1;
    uint64_t to = (n->size < max) ? n->size : max;
    if(to && n->data) move_read(n->data, 0, out, to);
    if(outLen) *outLen = to;
    return 0;
}
